ESPHome firmware for the DIYMORE ESP32 WIFI Plant Sensor

# How to install esphome to the DIYMORE ESP32 wifi plant sensor
...
<esp-web-install-button manifest="./diymore-plant-sensor-manifest.json"></esp-web-install-button>
...
# How to calibrate the DIYMORE ESP32 wifi plant sensor
...
```
[15:46:50][D][sensor:127]: '... Soil Conductivity': Sending state 0.07500 V with 2 decimals of accuracy
[15:46:46][D][sensor:127]: '... Soil Moisture': Sending state 2.83500 V with 2 decimals of accuracy
```
...
Open `DIYMORE-Plant-Sensor-ESP32.yaml` and find the following:
...
```
[16:22:22][D][sensor:127]: '... Soil Conductivity': Sending state 0.24400 V with 2 decimals of accuracy
[16:22:25][D][sensor:127]: '... Soil Moisture': Sending state 1.37300 V with 2 decimals of accuracy
```

So we write them down in the `DIYMORE-Plant-Sensor-ESP32.yaml` again but now the higher numbers.
...
Reflash it again with the previous command: `esphome run DIYMORE-Plant-Sensor-ESP32.yaml` and it is now calibrated!

You can also calibrate the temperature and humidity sensors, please read the `dht.yaml` to set the correct offsets.
